...
int main(){
    ...
    while (file >> temp)
    {
        ...
        if (firstEnd <= secondEnd && firstEnd >= secondStart || firstStart <= secondEnd && firstStart >= secondStart || firstStart >= secondStart && firstEnd <= secondEnd || secondStart >= firstStart && secondEnd <= firstEnd)
            counter++;
    }
    cout << counter << endl;
    return EXIT_SUCCESS;
